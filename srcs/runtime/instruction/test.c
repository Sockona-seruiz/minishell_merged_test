...
/*   Updated: 2021/03/17 10:25:41 by seruiz           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int     ft_strjoin_custom(t_shell_command cmd, char *new_str, char *new_mask, int var_value_len, int varname_len)
{
    ...
    final_mask = malloc(sizeof(char) * (total_len + 1));
    final_mask[total_len] = '\0';
...
int    ft_replace_in_struct(t_shell_command cmd, int i, char *varname)
{
    int j;
    int k;
    char    *new_str;
    ...
    j = 0;
    k = 0;
    var_value = ft_get_value(varname);
    printf("var_value = %s\n", var_value);
    new_str = malloc(sizeof(char) * (i + ft_strlen(var_value) + 1));
    new_str[i + ft_strlen(var_value)] = '\0';
    new_mask = malloc(sizeof(char) * (i + ft_strlen(var_value) + 1));
    new_mask[i + ft_strlen(var_value)] = '\0';
...
int    ft_replace_var(t_shell_command cmd, int i)
{
    char    mask;
    int     j;
    char    *varname;
    ...
    printf("varname = %s\n", varname);
    ft_replace_in_struct(cmd, i, varname);
    return (j + 1);
}
